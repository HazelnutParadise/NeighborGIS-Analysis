install.packages(c("tidygeocoder", "sf", "osmdata", "leaflet", "dplyr", "jsonlite", "readxl"))
...
poi_geo <- "/poi_geo_buffered.gpkg"
st_write(point_in_buffer, poi_geo, driver = "GPKG", delete_layer = TRUE)

# 讀取 GeoJSON
mrt_poi <- st_read("/poi_geo_buffered.gpkg") %>%
...
mrt_point_lines_sf <- mrt_poi %>%
  ...
  mutate(
    geometry_line = st_sfc(st_linestring(matrix(
      c(
        st_coordinates(.)[1], st_coordinates(.)[2],
