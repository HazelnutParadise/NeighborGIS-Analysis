...
plt.rcParams['font.sans-serif'] = ['Microsoft JhengHei'] # 或其他支援中文的字型如 'SimHei', 'Noto Sans CJK TC'
...
# Step 6. Leaflet 互動地圖
# ----------------------------------------------------------------------------
m = folium.Map(location=[25.04422, 121.5263], zoom_start=11, tiles="OpenStreetMap")
...
poly_land = land_public_simp[land_public_simp.geometry.geom_type.isin(['Polygon','MultiPolygon'])]
...
full_zone = full_zone[full_zone.geometry.geom_type.isin(['Polygon','MultiPolygon'])]
...
pts_pub = gpd.sjoin(
    pts_with_zone, poly_land,
    how="left",
    predicate='within'
).rename(columns={'Name':'公有土地名稱'})
...
# ----------------------------------------------------------------------------
m2 = folium.Map(location=[25.04422, 121.5263], zoom_start=11, tiles="CartoDB positron")
...
folium.GeoJson(
    ...
    style_function=lambda _: {
        'fillColor': 'red', 'color': 'black', 'weight':1, 'fillOpacity':0.5
    },
    highlight_function=lambda x: {'weight':5,'color':'#666','fillOpacity':0.9},
    tooltip=folium.GeoJsonTooltip(fields=['Name','Area'], aliases=['名稱','面積'])
).add_to(fg_public)
fg_public.add_to(m2)
...
folium.GeoJson(
    ...
    name='土地使用分區',
    style_function=lambda _: {
        'fillColor': 'black', 'color': 'black', 'weight':0.5, 'fillOpacity':0.3
    },
    tooltip=folium.GeoJsonTooltip(fields=['計畫_1','使用分'], aliases=['計畫區名稱','使用分區'])
).add_to(fg_zone)
fg_zone.add_to(m2)
